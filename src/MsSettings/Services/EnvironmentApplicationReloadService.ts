...
export class EnvironmentApplicationReloadService {
    constructor(private readonly urlGetSettings: string, private readonly applicationUuid: string) {
    }
    ...
    public async invoke(): Promise<void> {
        ...
        const {data} = await AxiosInstanceClient.get(`${this.urlGetSettings}`, {
            headers: {
                Authorization: Base64CredentialsHelper.builderUserCredentials(this.applicationUuid),
            }
        });
