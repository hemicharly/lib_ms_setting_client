...
class EnvironmentApplicationReloadService {
    ...
    static async reloadSecretsProcessEnv(key, secrets, applicationUuid) {
        if (secrets[key] && secrets[key] !== '') {
            process.env[key] = await Cryptography_1.cryptoDecryptAES.decrypt(secrets[key], applicationUuid);
        }
    }
}
